...
REM  Uso: scripts\windows\run.bat start ^| dev ^| install ^| reset
REM ==========================================================================
...
:RESET_DB
call :CABECALHO
set "DB_PATH=storage\database\kairo.sqlite"
if exist "%DB_PATH%" (
  ...
  set "BACKUP_PATH=storage\backups\reset-manual-!STAMP!.sqlite"
  call :BARRA "Criando backup de seguranca" 10
  copy /b "%DB_PATH%" "!BACKUP_PATH!" >nul
  if errorlevel 1 (
    echo   %VERM%[x] O backup falhou. O banco original foi preservado.%RST%
    exit /b 1
  )
  del /f /q "%DB_PATH%" "%DB_PATH%-shm" "%DB_PATH%-wal" 2>nul
  echo   %VERDE%[ok] Banco reiniciado com backup em !BACKUP_PATH!.%RST%
  echo   %CINZA%     Um novo espaco sera criado na proxima inicializacao.%RST%
) else (
