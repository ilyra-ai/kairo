...
| **38** | **Responsividade 100% mobile-first** de todo o app | UI | 🟠 EM ANDAMENTO (auditoria estática feita; falta QA navegado) | Solicitada em 22/07/2026. **Não criar novos componentes** — só tornar responsivo o existente. **Feito:** auditoria estática do CSS (base já sólida: `overflow-x:hidden` global, `box-sizing:border-box`, sidebar→drawer com hambúrguer, grids reduzem por breakpoint, tabelas com scroll, calendário com scroll horizontal); **reforços mobile** adicionados (grade de retenção 1 col ≤480px, heatmap 24h com rolagem, alvos ≥44px, tabelas contidas); widgets novos da 35 já mobile-first. **Falta:** QA navegado tela-a-tela em viewport real (depende do app no ar, como a Tarefa 24). |
| **13** | Gateways de pagamento e aplicação real dos planos | Comercial | 🟠 BACKEND CONCLUÍDO (22/07/2026); falta UI + chaves do gateway | **Módulo `payments` real e testado (5 testes):** `subscriptions` + `payment_events` (idempotência); `createCheckout` (cobrança pendente por provedor), **webhook assinado HMAC-SHA256** idempotente que **aplica o plano de verdade** (`users.plan`), `cancel` que reverte ao Free. Rotas `GET /api/payments/plans`, `POST /checkout`, `GET /subscription`, `POST /cancel` (auth+CSRF) e `POST /api/payments/webhook/:provider` (público, autenticado por assinatura). Provedores: stripe/mercadopago/manual. **Falta:** UI de assinatura no app e as **chaves reais do gateway** (`PAYMENTS_WEBHOOK_SECRET` + credenciais Stripe/MP) que **você** precisa configurar no ambiente. |
...
> - **✅ 35.12 — Ritual de Encerramento:** fecha o dia (inspirado no Deep Work shutdown) — `summary` revisa concluídos e pendências do dia e sugere o plano do amanhã (pendências de maior prioridade primeiro); `complete` persiste o plano do amanhã (limitado ao nº de itens do admin, upsert por data); `history` recupera a aderência. `GET /api/smart/shutdown/summary`, `POST /shutdown/complete`, `GET /shutdown/history`. **4 testes.**
>
> **✅ BACKEND DA TAREFA 35 — 100% (governança 35.0 + 12 engines reais e testados).** Todos os recursos são administráveis (ligar/desligar, parâmetros, vínculo de IA/artefato, dry-run, auditoria) e consumidos por rotas de usuário sob CSRF, cada engine barrando execução quando o recurso está desativado. Suíte atual: **160 testes** + **9 guardiões de frontend**.
>
